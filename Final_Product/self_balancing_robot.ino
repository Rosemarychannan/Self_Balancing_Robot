...
float offset_angle = 0.3; // balance setpoint
...
void reverse(int num, int pwm){
  if(num == A){
    if(turn_L < 0)
      analogWrite(AIN1,constrain(int((255 - pwm)*(-turn_L)), 0, 255));
    else if(turn_L == 1)
    ...
    analogWrite(BIN1,constrain(int((255-pwm)*turn_R), 0, 255));
    analogWrite(BIN2,255);
  }
}
...
void both_forward(int pwm){
  forward(A, pwm);
  forward(B, int(pwm*0.98));
}

void both_reverse(int pwm){
  reverse(A, pwm);
  reverse(B, int(pwm*0.98));
}
    ...
    while (central.connected()) {
      ...
      if (customCharacteristic.written()) {
       // Get the length of the received data
        ...
        if (strcmp((const char*)receivedString, "l") == 0) {
          turn_L = 1.1;
          turn_R = 1;
          kp_en = 0.1;
        }   
        else if (strcmp((const char*)receivedString, "r") == 0) {
          turn_L = -1.15;
          turn_R = 1;
          kp_en = 0.1;
        }
        ...
        else if (strcmp((const char*)receivedString, "ufp") == 0) {
          i_theta = 0;
          i_theta_en = 0;
        }
        else if (strcmp((const char*)receivedString, "dfn") == 0) {
          offset_angle -=2;
        }
        else if (strcmp((const char*)receivedString, "dfp") == 0) {
          offset_angle +=2;
        }
